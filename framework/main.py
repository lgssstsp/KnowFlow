...
parser.add_argument('--num_cases', type=int, default=5, help='None')
parser.add_argument('--knowledge_percentage', type=float, default=1.0, help='None')
parser.add_argument('--knowledge_types', nargs='+', default=['ogb','pyg_info_nn'], help='None')
...
while is_revise:
    ...
    manager_agent_start_time = time.time()
    manager_agent = ManagerAgent(retriever=retriever, args=args)   
    planning_results = manager_agent.execute(user_req)
    manager_agent_end_time = time.time()
    ...
    is_revise = manager_agent.revise_loop(planning_results, response)
